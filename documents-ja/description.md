...
これらのテストは全て ローカルの Visual Studio 2013, Team Foundation Server, Visual Studio Online 上で動作します。ビルド定義を作成して、「手動」あるいは「日次」で自動ビルド・自動テストを行う方法も解説します。これにより、チームの誰かがある日突然ソースコードを壊しても、すぐに発見することが出来るようになります。
...
Test で終わる名称は、TFS/VSO の自動ビルド・自動テストのために必要な命名規則です（変更も可能ですが、面倒です）。単体テストプロジェクトは必ず、対象の名前 .Test という名前をつけるようにします。
...
Microsoft Fakes は Visual Studio 2013 Premium 以上でしか使用できません。Fakes を使用するか、使用しなくても動作するクラスを実装するか？（後者をおすすめしますが）最初に検討する必要があるでしょう。
...
データベースは mdfファイル のアタッチ、DACPAC、BACPAC からのインポートの3種類の方法で初期化しています。初期データが入っているべきか？それともスキーマだけで良いのか？また、テスト時の実行速度は mdf が最高速で、対してDACPAC,　BACPAC はそれぞれ遅く、Visual Studio Online のビルド時間を多く消費してしまいます。それらを考慮した上で、データベースをどのように使用するか？検討します。
...
![チームエクスプローラ ホーム](description/6.png)

ビルド の「ビルド定義の新規作成」 を押すとビルド定義を作成出来ます。

![チームエクスプローラ ビルド定義](description/7.png)

![ビルド ソース設定](description/8.png)

ビルド定義の作成時にやらなければならないのは、対象とするプロジェクトの選択のみです。

チェックインポリシーを設定している場合は、そのためのルールセットも一緒にビルド対象に含める必要がある点に注意してください。

#### TFS Project Welcome pages（README.md）とチェックインポリシー に指定する静的コード分析 ルールセット
[https://code.msdn.microsoft.com/Welcome-pagesREADMEmd-75566110](https://code.msdn.microsoft.com/Welcome-pagesREADMEmd-75566110)

![ビルド プロセス](description/9.png)

既定で自動テストが *test*.dll という名称でビルドされるプロジェクトにのみ実行されます。

また、コード分析は設定に応じて実行されます。

名前を適切に設定したら保存してください。

![ビルド キューに追加](description/10.png)

保存したビルド定義を右クリックして、キューに追加することで新しいビルドを TFS/VSO で実行できます。

ローカルでは数秒で終わるこのサンプルのテストですが、Visual Studio Online での実行にはおおよそ10分程度かかります。キューに積まれてから実際に実行されるまで時間がかかる場合があります。

![ビルド結果](description/11.png)

テストが成功するとこのように Visual Studio で確認することが出来ます。Web 画面でも確認できます。

![ビルド結果 Web](description/12.png)

Web 画面で BUILD からビルド定義を選択して、「Pin to homepage」を押すと HOME Overview（ホーム・概要）に表示しておくことが出来ます。

![ビルド結果 Home](description/13.png)

ビルド定義を「手動」から「日次」に変更すると、毎日自動ビルド・自動テストを行ってくれます。（BUILD には無料時間枠がありますが、それを超えると有料になるか、ビルドが出来なくなるので注意が必要してください）
...
- Visual Studio Online で失敗したときも、詳細に内容が表示されます。Visual Studio Online の日次ビルドを構成して、ホーム画面にピンしましょう。 
